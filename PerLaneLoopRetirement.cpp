...
#define GEN_PASS_DECL_NVPERLANELOOPRETIREMENT
...
struct NVPerLaneLoopRetirement
    : public mlir::triton::plugin::impl::NVPerLaneLoopRetirementBase<
...
static void addPerLaneRetirePass(mlir::PassManager *pm,
                                 const std::vector<std::string> &args) {
  pm->addPass(mlir::triton::plugin::createNVPerLaneLoopRetirement());
}

static void registerPerLaneRetirePass() {
  ::mlir::registerPass([]() -> std::unique_ptr<::mlir::Pass> {
    return mlir::triton::plugin::createNVPerLaneLoopRetirement();
  });
}
...
TRITON_PLUGIN_API plugin::PluginInfo *tritonGetPluginInfo() {
  static plugin::PassInfo pass = {"nv_per_lane_loop_retirement", "0.1.0",
                                  addPerLaneRetirePass,
                                  registerPerLaneRetirePass};
  static plugin::PassInfo passes[] = {pass};
  static plugin::PluginInfo info = {TRITON_PLUGIN_API_VERSION,
                                    "PerLaneLoopRetirement",
                                    "0.1.0",
                                    passes,
                                    1,
                                    nullptr,
